...
namespace Channel
{
	public class ChannelServer : ServerMain
	{
		...
		public void ConnectToLogin(bool firstTime)
		{
			...
			while (!success)
			{
				try
				{
					...
					this.LoginServer.Crypto = new CoreCrypto(BitConverter.ToUInt32(buffer, 0), false);
		...
		/// </summary>
		/// <remarks>
		/// When calculating the <see cref="ChannelState"/> we take into account whether the server is running as well as if it is in Maintenance.
		/// </remarks>
		...
		public override void InitDatabase(SharedDb db, BaseConf conf)
		{
			base.InitDatabase(db, conf);

			// If items end up with temp ids in the db we'd get entity ids  that exist twice, when creating new temps later on.
		...
		public void Shutdown(int time)
		{
			...
			// Add a few seconds, as `time` is the moment when all clients send their DC request, because of RequestClientDisconnect.
			// The channel should shutdown *after* that's done. 10 seconds should be plenty.
			time += 10;
		...
		private void ShutdownTimerDone(object timer)
		{
			...
			this.Database.SaveVars("Server System", 0, this.ScriptManager.GlobalVars.Perm);
