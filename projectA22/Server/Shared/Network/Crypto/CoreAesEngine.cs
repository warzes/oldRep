namespace Shared.Network.Crypto
{
	...
	/// </summary>
	public sealed class CoreAesEngine
	{
		/// <summary>
		/// Any data left over after processing all multiples of 16 is XORed with this array.
		/// </summary>
		...
		public CoreAesEngine(bool forEncryption, byte[] key)
		{
		...
		public unsafe void ProcessPacket(byte[] packet, int offset, int count)
		{
			...
			fixed (byte* tmpPtr = temp, packetPtr = packet)
			{
				...
				while (ctr >= 16)
				{
					// cipher treats blocks as Big Endian
